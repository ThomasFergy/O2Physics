...
struct strangeness_tutorial {
  ...
  HistogramRegistry rXi{
      "xi", {}, OutputObjHandlingPolicy::AnalysisObject, true, true};
  ...
  Configurable<double> cascadesetting_cospa{
      "cascadesetting_cospa", 0.98,
      "Casc CosPA"};  // double -> N.B. dcos(x)/dx = 0 at x=0
  Configurable<float> cascadesetting_dcacascdau{"cascadesetting_dcacascdau",
                                                1.0, "DCA cascade daughters"};
  Configurable<float> cascadesetting_dcabachtopv{"cascadesetting_dcabachtopv",
                                                 0.1, "DCA bachelor to PV"};
  Configurable<float> cascadesetting_mindcav0topv{"cascadesetting_mindcav0topv",
                                                  0.01, "minimum V0 DCA to PV"};
  Configurable<float> cascadesetting_cascradius{"cascadesetting_cascradius",
                                                0.5, "cascradius"};
  Configurable<float> cascadesetting_v0masswindow{"cascadesetting_v0masswindow",
                                                  0.01, "v0 mass window"};
  ...
  void init(InitContext const&) {
    ...
    rKzeroShort.add("hMassK0ShortSelectedTruePions",
                    ...
                    {HistType::kTH1F, {{200, 0.45f, 0.55f}}});
    ...
    rXi.add("hMassXiSelected", "hMassXiSelected",
            {HistType::kTH1F, {XiMassAxis}});
    rXi.add("hMassXiTrueRec", "hMassXiTrueRec",
            {HistType::kTH1F, {XiMassAxis}});
    ...
    // Xi topological cuts
    rXi.add("hCascDCAV0Daughters", "hCascDCAV0Daughters",
            {HistType::kTH1F, {{55, 0.0f, 2.2f}}});
  ...
  using DaughterTracks =
      soa::Join<aod::TracksIU, aod::TracksExtra, aod::pidTPCPi, aod::pidTPCPr,
                aod::McTrackLabels>;
  ...
  void processRecMC(
    ...
    for (const auto& casc : Cascades) {
      ...
      // Cut on dynamic columns
      if (casc.casccosPA(collision.posX(), collision.posY(), collision.posZ()) <
          cascadesetting_cospa)
        continue;
      if (TMath::Abs(casc.mLambda() - pdgDB->Mass(3122)) >
          cascadesetting_v0masswindow)
        continue;
      if (casc.dcav0topv(collision.posX(), collision.posY(), collision.posZ()) <
          cascadesetting_mindcav0topv)
        continue;
      if (casc.cascradius() < cascadesetting_cascradius) continue;
